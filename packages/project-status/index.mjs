...
export function validateCutoverStage(dash, cut, acceptance, active = false) {
  check((active ? ['in_progress','done'] : ['planned','in_progress','review']).includes(cut), 'CUT-Status nicht autorisiert');
...
export function buildSnapshot({ repo, vault, register }) {
  ...
  validateCutoverStage(gates[16].state, gates[17].state,
    gates[17].state === 'planned' ? null : readJSON('roadmap/v2/dashboard/acceptance.json'), baseline.activeBaseline === 'v2');
  if (gates[17].state !== 'planned') {
    // The current CUT contract is additive. Historical V2 seals remain intact.
    if (!hasDecision) execFileSync(process.platform === 'win32' ? 'python.exe' : 'python', ['-B','scripts/validate_v2_cutover.py'],
