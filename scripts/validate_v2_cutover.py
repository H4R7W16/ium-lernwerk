...
def _validate(repo, d, vault, require_verified):
    ...
    baseline = read(repo, 'roadmap/v2/status.json')
    ...
    manifest = read(repo, 'roadmap/v2/dashboard/gates.json')[:-1]
    ...
    acceptance = read(repo, 'roadmap/v2/dashboard/acceptance.json')
    ...
    requirements = read(repo, 'roadmap/v2/requirements/requirements.json')['requirements']
    ...
    expected = {}
    lxf = read(repo, 'roadmap/v2/foundations/learning-experience/status.json')['review']['openQuestions']
    gov = read(repo, 'roadmap/v2/foundations/governance/governance-contract.json')['openQuestions']
    for q in read(repo, 'roadmap/v2/audits/status.json')['carryForward']:
        source = next(x for x in lxf + gov if x['id'] == q['id'])
        expected[q['id']] = ('foundation', 'open', source)
    for grade in [5, 6, 7]:
        for q in read(repo, f'roadmap/v2/grades/grade-{grade}/roadmap.json')['entryGates']:
    ...
    progression = read(repo, 'roadmap/v2/grades/grade-7/progression.json')
    for q in progression['privacyCarry'] + progression['newOpenReflectionIds']:
        expected[q] = ('curriculum-evidence', 'open', {})
    for q in read(repo, 'roadmap/v2/audits/follow-up-tasks.json')['tasks']:
    ...
    require(not (repo / activation['proposedCurrentPointer']).exists(), 'Aktivierung gehört nicht in die Entscheidungsvorbereitung')
    ...
    require(set(d['inputDigests']) == set(input_paths(repo)), 'Aktuelle Inputbindung unvollständig')
    for path, expected_digest in d['inputDigests'].items():
        require(expected_digest == current_digest((repo / path).read_bytes()), 'Aktueller Input verändert: ' + path)


def validate(repo, packet=None, *, vault=None, require_verified=False):
    try:
        _validate(repo, read(repo, PACKET) if packet is None else packet, vault, require_verified)
...
if __name__ == '__main__':
    ...
    try:
        validate(ROOT, vault=args.vault, require_verified=args.require_verified)
    except CutoverError as exc:
        parser.exit(1, str(exc) + '\n')
    print('CUT-Vertrag gültig: 17 Freigaben, 8 Anforderungen, 46 Bedingungseinträge. Nutzerentscheidung ausstehend; V1 aktiv.')
